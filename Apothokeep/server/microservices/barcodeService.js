...
const { FoodstuffService } = require("../services/foodstuffService.ts");
...
wss.on("connection", (ws, req) => {
  ...
  ws.on("message", async (data) => {
    ...
    try {
      ...
      const baseName = nameObj?.Name ?? "Unknown";
      const name = subObj?.Name_subtitle ? `${baseName} (${subObj.Name_subtitle})` : baseName;

      const doc = await FoodstuffService.create({
        name,
        opened,
        purchaseDate,
        expirationDate,
